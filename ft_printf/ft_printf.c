...
static int	print_this(va_list args, char c )
{
	int	count;
	...
	if (c == 'p')
		count += fi_putptr(va_arg(args, unsigned long));
	if (c == 'd' || c == 'i')
		count += fi_putnbr(va_arg(args, int));
	if (c == 'u')
		count += fi_putuint(va_arg(args, unsigned int));
	if (c == 'x')
		count += fi_puthex(va_arg(args, unsigned int), 'x');
	if (c == 'X')
		count += fi_puthex(va_arg(args, unsigned int), 'X');
	if (c == '%')
		count += fi_putchar('%');
